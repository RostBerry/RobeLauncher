namespace Chess {
    public static class MagicBitboardsData {
        public static readonly ulong[] RookMovesMask; // Bitboards for all 64 squares with all available moves for the rook as if the board was empty
        public static readonly ulong[] BishopMovesMask; // Same as RookMovesMask but for bishops

        public static readonly ulong[][] RookLookupTable; // Bitboards for legal moves of all possible rook and blockers combinations
        public static readonly ulong[][] BishopLookupTable; // Same but bishops
        ...
        static MagicBitboardsData() {
            RookMagicNumbers = new ulong[64] {
            ...
            };  // Magic numbers have been entirely stolen from Sebastian Lague
            BishopMagicNumbers = new ulong[64] {
            ...
            };  // These ones too, sorry, Sebastian
            ...
            RookMovesMask = new ulong[64];
            BishopMovesMask = new ulong[64];

            RookLookupTable = new ulong[64][];
            BishopLookupTable = new ulong[64][];

            for (int square = 0; square < 64; square++) {
                RookMovesMask[square] = MagicBitboardsGenerator.GenerateMovesMask(square, false);
                BishopMovesMask[square] = MagicBitboardsGenerator.GenerateMovesMask(square, true);

                RookLookupTable[square] = GenerateLookupTable(square, false, RookMagicNumbers[square], RookShifts[square]);
                BishopLookupTable[square] = GenerateLookupTable(square, true, BishopMagicNumbers[square], BishopShifts[square]);
            }
        }

        private static ulong[] GenerateLookupTable(int square, bool isBishop, ulong magicNumber, int shiftAmount) {
            int bitCount = 64 - shiftAmount;
            int tableSize = 1 << bitCount;
            ulong[] lookupTable = new ulong[tableSize];

            ulong movesMask = isBishop? BishopMovesMask[square]: RookMovesMask[square];
            ulong[] blockerPatterns = MagicBitboardsGenerator.CreateBlockersBB(movesMask);

            foreach(ulong pattern in blockerPatterns) {
                ulong tableIndex = (pattern* magicNumber) >> shiftAmount;
                ulong legalMovesBB = MagicBitboardsGenerator.GenerateLegalMoves(square, pattern, isBishop);
                lookupTable[tableIndex] = legalMovesBB;
            }

            return lookupTable;
        }

        public static ulong GetRookLegalMoves(int square, ulong allPieces) {
            ulong key = ((allPieces & RookMovesMask[square]) * RookMagicNumbers[square]) >> RookShifts[square];
            return RookLookupTable[square][key];
        }

        public static ulong GetBishopLegalMoves(int square, ulong allPieces) {
            ulong key = ((allPieces & BishopMovesMask[square]) * BishopMagicNumbers[square]) >> BishopShifts[square];
            return BishopLookupTable[square][key];
        }

        public static ulong GetRayPieceLegalMoves(int square, ulong allPieces, bool isBishop) {
            if (isBishop) {
                return GetBishopLegalMoves(square, allPieces);
            }
            return GetRookLegalMoves(square, allPieces);
        }
    }
}